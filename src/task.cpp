...
void TaskManager::updateTaskDetails(Task& task) {
    std::cout << "Enter new description (leave blank to keep current): ";
    std::string desc;
    std::getline(std::cin, desc);
    if (!desc.empty()) task.description = desc;

    std::cout << "Enter new priority (Low, Medium, High, leave blank to keep current): ";
    std::string priority;
    std::getline(std::cin, priority);
    if (!priority.empty()) {
        task.priority = priority; // Consider converting to enum if applicable
    }
